...
//   8. 출력: Xgen_Manual/base/user/images/admin-role-permissions-modal.gif
...
const TMP_DIR = path.join(REPO_ROOT, '.tmp-gs-role-perm');
...
(async () => {
  ...
  await page.waitForFunction(
    () => /시스템\s*관리자\s*\(DJ\)/.test(document.body.innerText),
    null,
    { timeout: 25_000 },
  );
  const tEmptyReady = Date.now() - ctxStart;
  log(`  role table ready at t=${tEmptyReady}ms`);

  await page.mouse.move(200, 300);
  await page.waitForTimeout(PRE_CLICK_HOLD_MS);

  // ── 4. 시스템 관리자(DJ) 행의 권한 버튼까지 이동 → 클릭 → 모달 ──
  const permRect = await page.evaluate(() => {
    const trows = Array.from(document.querySelectorAll('tr'));
    const djRow = trows.find(tr => /시스템\s*관리자\s*\(DJ\)/.test(tr.textContent));
    if (!djRow) return null;
    djRow.scrollIntoView({ block: 'center' });
    const permBtn = Array.from(djRow.querySelectorAll('button')).find(b => /^권한$/.test(b.textContent.trim()));
    if (!permBtn) return null;
    const r = permBtn.getBoundingClientRect();
    return { x: r.x + r.width / 2, y: r.y + r.height / 2 };
  });
  if (!permRect) throw new Error('DJ 행 권한 버튼 미발견');
  log(`  cursor → DJ 권한 (${permRect.x.toFixed(0)},${permRect.y.toFixed(0)})`);
  await page.mouse.move(permRect.x, permRect.y, { steps: 25 });
  ...
  log('  권한 clicked → modal');
  await page.waitForFunction(() => !!document.querySelector('[role="dialog"]'), null, { timeout: 8_000 });
  await page.waitForTimeout(800);

  // ── 5. ADMIN 펼치기 → AI 거버넌스 체크 ──
  const adminRect = await page.evaluate(() => {
    const dlg = document.querySelector('[role="dialog"]');
    const cands = Array.from(dlg.querySelectorAll('[cursor="pointer"], div')).filter(el => /^ADMIN/.test(el.textContent.trim()) && /\d+개 카테고리/.test(el.textContent));
    const cand = cands[0];
    if (!cand) return null;
  ...
  if (adminRect) {
    log(`  cursor → ADMIN accordion (${adminRect.x.toFixed(0)},${adminRect.y.toFixed(0)})`);
    await page.mouse.move(adminRect.x, adminRect.y, { steps: 20 });
  ...
  // AI 거버넌스 체크박스 클릭
  const govRect = await page.evaluate(() => {
    const dlg = document.querySelector('[role="dialog"]');
    const cands = Array.from(dlg.querySelectorAll('div')).filter(el => /^AI\s*거버넌스/.test(el.textContent.trim()) && el.textContent.length < 50);
    const cand = cands.find(el => el.getBoundingClientRect().width > 100) || cands[0];
    if (!cand) return null;
    cand.scrollIntoView({ block: 'center' });
    const r = cand.getBoundingClientRect();
    return { x: r.x + 30, y: r.y + r.height / 2 }; // 왼쪽 가까이 = 체크박스 위치
  });
  if (govRect) {
    log(`  cursor → AI 거버넌스 (${govRect.x.toFixed(0)},${govRect.y.toFixed(0)})`);
    await page.mouse.move(govRect.x, govRect.y, { steps: 18 });
    await page.waitForTimeout(180);
    await page.evaluate(() => {
      const dlg = document.querySelector('[role="dialog"]');
      const cands = Array.from(dlg.querySelectorAll('div')).filter(el => /^AI\s*거버넌스/.test(el.textContent.trim()) && el.textContent.length < 50);
  ...
  // ── 6. ADMIN 접고 MAIN 펼치기 → 지식관리 체크 해제 ──
  await page.evaluate(() => {
    const dlg = document.querySelector('[role="dialog"]');
    const adminAcc = Array.from(dlg.querySelectorAll('div')).find(el => /^ADMIN/.test(el.textContent.trim()) && /\d+개 카테고리/.test(el.textContent));
    adminAcc?.click(); // collapse
  });
  await page.waitForTimeout(500);

  const mainRect = await page.evaluate(() => {
    const dlg = document.querySelector('[role="dialog"]');
    const cands = Array.from(dlg.querySelectorAll('div')).filter(el => /^MAIN/.test(el.textContent.trim()) && /\d+개 카테고리/.test(el.textContent));
    const cand = cands[0];
    if (!cand) return null;
    const r = cand.getBoundingClientRect();
    return { x: r.x + r.width / 2, y: r.y + r.height / 2 };
  ...
  if (mainRect) {
    log(`  cursor → MAIN accordion (${mainRect.x.toFixed(0)},${mainRect.y.toFixed(0)})`);
    await page.mouse.move(mainRect.x, mainRect.y, { steps: 20 });
    await page.waitForTimeout(180);
    await page.evaluate(() => {
      const dlg = document.querySelector('[role="dialog"]');
      const cands = Array.from(dlg.querySelectorAll('div')).filter(el => /^MAIN/.test(el.textContent.trim()) && /\d+개 카테고리/.test(el.textContent));
  ...
  // 지식관리 체크박스 클릭 (체크 해제)
  const knowRect = await page.evaluate(() => {
    const dlg = document.querySelector('[role="dialog"]');
    const cands = Array.from(dlg.querySelectorAll('div')).filter(el => /^지식\s*관리/.test(el.textContent.trim()) && el.textContent.length < 50);
    const cand = cands.find(el => el.getBoundingClientRect().width > 100) || cands[0];
    if (!cand) return null;
    cand.scrollIntoView({ block: 'center' });
    const r = cand.getBoundingClientRect();
  ...
  if (knowRect) {
    log(`  cursor → 지식관리 (${knowRect.x.toFixed(0)},${knowRect.y.toFixed(0)})`);
    await page.mouse.move(knowRect.x, knowRect.y, { steps: 18 });
    await page.waitForTimeout(180);
    await page.evaluate(() => {
      const dlg = document.querySelector('[role="dialog"]');
      const cands = Array.from(dlg.querySelectorAll('div')).filter(el => /^지식\s*관리/.test(el.textContent.trim()) && el.textContent.length < 50);
  ...
  const gifPath = path.join(OUT_DIR, 'admin-role-permissions-modal.gif');
