...
const TMP_DIR = path.join(REPO_ROOT, '.tmp-gs-transition-gif');
...
const SHOTS = [
  ...
  // plan-new (Agent 기획) — gs-cert 에서는 main-planning view 미노출이므로 SHOT 제외.
  ...
  // quality-harmbench — Harmbench 평가 트리거 SHOT 은 main 환경에 미노출되어 gs-cert 에선 제외.
  ...
  try {
    for (const shot of shotsToRun) {
      ...
      if (!account?.email || !account?.pass) {
        throw new Error(`missing credentials for account=${shot.account} (set XGEN_${shot.account.toUpperCase()}_EMAIL/PASSWORD in .env.gs-cert)`);
      }
