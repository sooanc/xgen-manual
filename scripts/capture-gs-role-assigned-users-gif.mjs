...
//   8. 출력: Xgen_Manual/base/user/images/admin-role-assigned-users.gif
...
const TMP_DIR = path.join(REPO_ROOT, '.tmp-gs-role-assigned-users');
...
(async () => {
  ...
  // ── 4. 시스템 관리자(DJ) 행의 사용자 버튼까지 이동 → 클릭 → 사용자 관리 모달 ──
  const userRect = await page.evaluate(() => {
    ...
    const userBtn = Array.from(djRow.querySelectorAll('button')).find(b => /^사용자$|^Users$/.test(b.textContent.trim()));
    if (!userBtn) return null;
    const r = userBtn.getBoundingClientRect();
    return { x: r.x + r.width / 2, y: r.y + r.height / 2 };
  });
  if (!userRect) throw new Error('DJ 행 사용자 버튼 미발견');
  log(`  cursor → DJ 사용자 (${userRect.x.toFixed(0)},${userRect.y.toFixed(0)})`);
  await page.mouse.move(userRect.x, userRect.y, { steps: 25 });
  ...
  // ── 5. 모달 hold (할당된 사용자 chip + 사용 가능 목록) ──
  await page.waitForTimeout(POST_CLICK_HOLD_MS + 1800);
  ...
  const gifPath = path.join(OUT_DIR, 'admin-role-assigned-users.gif');
