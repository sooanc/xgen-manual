// capture-gs-admin.mjs
// xgen.x2bee.com 라이브 화면을 gs-cert customer overlay 로 캡처 (관리자 모드).
//
//   cd C:\xgen-manual && node scripts/capture-gs-admin.mjs
//
// 자격증명: .env.gs-cert (GS인증 계정 gs@plateer.com — 사용자/관리 양쪽 모드 가능)
// 출력 위치: Xgen_Manual/customers/gs-cert/overlay/admin/images/
// screen-truth: Xgen_Manual/customers/gs-cert/screen-truth.json
//
// compose.mjs 가 본 디렉토리를 base/admin/images/ 위에 overlay 하여 gs-cert 매뉴얼
// 빌드 결과에 GS인증 관리자명이 노출된 캡처가 들어가도록 합니다.
...
// 캡처 양쪽이 같은 파일에 머지 기록.
const SCREEN_TRUTH_FILE = path.join(REPO_ROOT, 'Xgen_Manual', 'customers', 'gs-cert', 'screen-truth.json');
