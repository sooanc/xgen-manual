...
//   8. 출력: Xgen_Manual/base/user/images/20 canvas agent running.gif
...
const TMP_DIR = path.join(REPO_ROOT, '.tmp-gs-canvas-agent-running');
...
(async () => {
  ...
  // ── 3. Agent 목록 페이지 진입 (사내규정 QA챗봇 카드 확인) ──
  log('  navigating to /main?view=agentflows');
  await page.goto(`${BASE}/main?view=agentflows`, { waitUntil: 'networkidle', timeout: 45_000 });
  await page.waitForFunction(
    () => /사내규정\s*QA\s*챗봇/.test(document.body.innerText),
    null,
    { timeout: 20_000 },
  );
  const tListReady = Date.now() - ctxStart;
  log(`  agent list ready at t=${tListReady}ms`);

  // 사내규정 QA챗봇 카드 안의 편집 버튼 좌표 가져오기 + 스크롤
  const editRect = await page.evaluate(() => {
    const cards = Array.from(document.querySelectorAll('div')).filter(d =>
      /사내규정\s*QA\s*챗봇/.test(d.textContent) && d.textContent.length < 200,
    );
    const card = cards.sort((a, b) => a.textContent.length - b.textContent.length).pop();
    if (!card) return null;
    const editBtn = Array.from(card.querySelectorAll('button')).find(b => /^편집$/.test(b.textContent.trim()));
    if (!editBtn) return null;
    editBtn.scrollIntoView({ block: 'center', behavior: 'instant' });
    const r = editBtn.getBoundingClientRect();
    return { x: r.x + r.width / 2, y: r.y + r.height / 2 };
  });
  if (!editRect) throw new Error('사내규정 QA챗봇 편집 버튼 미발견');

  // 시작 상태를 잠시 보여주기
  await page.mouse.move(200, 200);
  await page.waitForTimeout(PRE_CLICK_HOLD_MS);

  // ── 4. 편집 버튼까지 부드럽게 이동 → 클릭 ──
  log(`  moving cursor to 편집 button (${editRect.x.toFixed(0)},${editRect.y.toFixed(0)})`);
  await page.mouse.move(editRect.x, editRect.y, { steps: 22 });
  ...
  log('  편집 clicked → canvas load');

  // ── 5. 캔버스 로드 + 70% 줌 표시 대기 ──
  await page.waitForURL((u) => u.toString().includes('/canvas?load='), { timeout: 30_000 });
  await page.waitForFunction(
    () => /\d+%/.test((document.querySelector('[role="status"]') || {}).textContent || ''),
    null,
    { timeout: 25_000 },
  );
  ...
  const expandRect = await page.evaluate(() => {
    const btn = Array.from(document.querySelectorAll('button')).find(b => (b.getAttribute('aria-label') || '').includes('펼치기'));
    if (!btn) return null;
    const r = btn.getBoundingClientRect();
    return { x: r.x + r.width / 2, y: r.y + r.height / 2 };
  });
  if (expandRect) {
    log(`  cursor → 펼치기 (${expandRect.x.toFixed(0)},${expandRect.y.toFixed(0)})`);
    await page.mouse.move(expandRect.x, expandRect.y, { steps: 22 });
    await page.waitForTimeout(150);
    await page.evaluate(() => {
      const btn = Array.from(document.querySelectorAll('button')).find(b => (b.getAttribute('aria-label') || '').includes('펼치기'));
      btn?.click();
  ...
  // ── 8. 실행 버튼 클릭 → 메시지 + 완료 내용 캡처 ──
  const runRect = await page.evaluate(() => {
    const btn = Array.from(document.querySelectorAll('button')).find(b => /^실행$/.test(b.textContent.trim()));
    if (!btn) return null;
    const r = btn.getBoundingClientRect();
    return { x: r.x + r.width / 2, y: r.y + r.height / 2 };
  });
  if (runRect) {
    log(`  cursor → 실행 (${runRect.x.toFixed(0)},${runRect.y.toFixed(0)})`);
    await page.mouse.move(runRect.x, runRect.y, { steps: 22 });
    await page.waitForTimeout(200);
    await page.evaluate(() => {
      const btn = Array.from(document.querySelectorAll('button')).find(b => /^실행$/.test(b.textContent.trim()));
      btn?.click();
    });
    log('  실행 clicked — waiting for log output');
  }

  // ── 9. 실행 메시지 + 완료 hold (충분히 보여주기) ──
  await page.waitForTimeout(POST_CLICK_HOLD_MS + 7000);
  const tEnd = Date.now() - ctxStart;
  log(`  total recorded duration ≈ ${tEnd}ms`);
  const tEmptyReady = tListReady; // canvas-start-gif 호환 변수 alias
  ...
  const gifPath = path.join(OUT_DIR, '20 canvas agent running.gif');
