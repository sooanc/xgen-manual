...
3. **제외 방식**: 해당 디렉토리에 `.pages` (awesome-pages) 가 **없을 때**만 자동 생성 — 알파벳 순으로 남은 챕터를 명시. 이미 `.pages` 가 있으면 수동 nav 우선이라 건드리지 않고 로그만 남김.
4. **페이지는 남김**: `.md` 자체는 docs/ 에 그대로 복사되므로 직접 URL 로 접근 가능 (외부에서 옛 링크로 들어오는 경우 대비).
...
- 자동 생성된 `.pages` 위치: `.build/composed/<customer>/docs/<section>/.pages` — 상단 코멘트에 제외 챕터명 명시.
