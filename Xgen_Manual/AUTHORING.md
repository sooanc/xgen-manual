...
1. **메인 진입**: [업무별 가이드](base/tasks/index.md) — 4개 역할(일반 사용자 / Agent 개발자 / 시스템 관리자 / 거버넌스 담당자) 기준 Task → 챕터 매핑
2. **보조 진입**: 공통 / 사용자 / 관리자 탭 — 메뉴 구조 기준 상세 레퍼런스
3. 홈 페이지([base/index.md](base/index.md))는 업무별 가이드 진입을 시각적으로 가장 강조

새 챕터·새 기능을 추가하면 반드시 **업무별 가이드의 해당 역할 절**에도 "이 작업이 필요하다면 → 이 챕터" 항목을 추가한다. 메뉴 챕터만 만들고 Task 매핑을 빼면 사용자 만족도가 낮아진다.
...
- 2026-05-13: 8.1 절 신설 — Task 우선 / 메뉴 보조 구조 원칙. 신규 [base/tasks/](base/tasks/) 탭 추가. 홈 페이지가 업무별 가이드를 메인 진입으로 안내.
