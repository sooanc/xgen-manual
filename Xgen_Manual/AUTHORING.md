...
### 8.3 문장·표현 출처 — base/ (stg 기준) 우선 { #base-phrasing-source }

새 매뉴얼 콘텐츠(고객사·환경별 customer, 신규 챕터, 절 단위 보강 등) 를 작성할 때는 AI 어시스턴트가 *처음부터* 문장을 짜내지 말고, **base/ 디렉토리(=xgen-standard / stg-xgen.x2bee.com 기준) 의 검증된 문장·표현·구조를 최대한 그대로 복사·재활용** 한다.

**근거**: base/ 의 문장은 시간이 지나며 사람이 다듬어온 자료라 문체·용어·뉘앙스가 안정적이고 매끄럽다. 반면 AI 가 처음부터 생성한 한국어 매뉴얼 문장은 호흡·존댓말 패턴·정형 표현에서 미세하게 어긋나 가독성이 떨어진다. customer 마다 prose 를 재작성하면 품질이 떨어지고 매뉴얼 간 일관성도 무너진다.

**적용 규칙**:

- **fork-on-create customer 의 prose 변경 최소화** — `customers/<id>/manual/` 트리는 base/ 의 카피이므로, *갭이 있는 부분만* 정밀 패치(제거·교체·추가) 하고 나머지 문장은 손대지 않는다. "더 매끄럽게 다시 써보자" 충동 금지.
- **신규 챕터를 만들 때** — base/ 안에서 *가장 비슷한 챕터* 를 찾아 그 구조·도입부·서식·"문의" 마무리 등을 그대로 모방한다. 새 챕터의 문체가 다른 챕터와 동떨어지지 않도록.
- **갭 처리 시 표 행·절 단위 수정** — 챕터 통째 보다는 줄 단위 패치가 base/ 의 문장을 최대한 살린다. require_view frontmatter 로 챕터 단위 제외 + 인라인 `<!-- require_view: <id> -->` 마커로 표 행·짧은 절 단위 제외 (자세한 메커니즘은 7a 절 참고).
- **새 문장이 꼭 필요할 때** — base/ 에서 *유사 맥락의 문장* 을 찾아 어휘·종결·존댓말 패턴(`~합니다`, `~할 수 있습니다`, `~를 선택하면 …`) 을 맞춘다. "본 챕터는 ~를 다룹니다", "좌측 사이드바 ~ 를 선택하면", "운영 권장사항", "문의: Xgen 솔루션 관리자" 같은 정형 패턴은 그대로 유지.
- **xgen-main 처럼 base/ 를 직접 공유하는 customer 는 자동으로 본 원칙 충족** — 환경별 갭은 `customers/<id>/screen-truth.json` + require_view 메커니즘으로 *제거만* 일어나고, prose 는 base/ 그대로 유지된다.

### 8.4 톤 — 고객 친화·구체적 설명 { #customer-friendly-tone }

모든 매뉴얼 본문은 **고객(엔드 사용자) 시점** 에서 *친절하고 이해하기 쉽게, 구체적으로* 설명한다. 시스템 관점·개발자 관점의 압축 서술 대신 *사용자가 그 화면 앞에서 무엇을 보고 어떻게 진행하는지* 를 단계별로 풀어쓴다.

**근거**: 매뉴얼 독자는 시스템 내부 구조를 모르는 일반 사용자·관리자입니다. "한 줄로 요약" 문체는 작성자에겐 효율적이어도 독자에겐 *클릭할 위치·다음 단계가 모호* 해 결국 다시 문의가 들어옵니다. 구체성이 곧 사용자 만족도입니다.

**적용 규칙**:

- **고객 시점 동사** — "시스템이 ~를 처리합니다" 보다 "**좌측 사이드바 ~ 를 선택하면 ~ 화면이 열립니다**" 처럼 *사용자가 행동의 주체* 가 되도록 서술합니다. 수동태·시스템 주어를 남발하지 않습니다.
- **친절한 존댓말 일관** — `~합니다 / ~할 수 있습니다 / ~해 주세요` 격식체로 통일합니다. 단답·명령형(`~할 것`, `~한다`) 은 AUTHORING.md / TONE_AND_MANNER.md 같은 *메타 문서에만* 사용하고 본문에서는 금지합니다.
- **구체적 라벨·진입 경로** — "메뉴를 엽니다" → "*좌측 사이드바 **관리 설정 → 사용자 관리** 를 선택합니다*" 처럼 *클릭 위치 · 라벨 · 다음에 보이는 것* 을 한 문장에 함께 적습니다. base/ 의 정형 패턴 ("좌측 사이드바 …", "화면 상단의 …", "행 우측의 *…* 버튼을 클릭하면") 을 그대로 활용합니다.
- **추상적 단어 풀어쓰기** — "최적화", "처리됨", "반영됨" 같은 흐릿한 단어 대신 *무엇이 어떻게 바뀌는지* 를 구체화합니다. 예: "백그라운드에서 처리됨" → "*문서가 큰 경우 임베딩 처리에 시간이 걸립니다. 진행률은 **업로드 이력** 탭에서 확인 가능. 처리 완료 전에도 다른 작업은 가능합니다.*" (base/user/15-knowledge.md 의 실제 표현).
- **숫자·시간·상태 명시** — "잠시 후 반영" 같은 막연한 표현 대신 "*수 초 내 반영*", "*최대 5분 이내*" 처럼 *사용자가 체감 가능한 단위* 로 적습니다. 가변적이면 "*문서 크기에 따라 시간이 더 소요될 수 있습니다*" 처럼 *왜 가변적인지* 도 함께 안내합니다.
- **운영 권장사항·주의 케이스 포함** — 단순 사용법 외에 *언제 이 기능을 쓰면 좋은지·어떤 상황에서 주의해야 하는지* 를 1-2줄 보강합니다. base/ 의 챕터 말미 "운영 권장사항" 절과 `!!! warning "..."` admonition 이 표준 패턴입니다.
- **시각 자료 보조** — 핵심 진입 동작·상태 전이는 스크린샷·GIF 로 보조합니다 (캡처 워크플로는 6 절). 본문은 화면을 *보지 않고도 따라할 수 있게* 적되, 화면이 있으면 더 빠르게 이해되도록 합니다.

표현 패턴의 세부 규칙(금칙어·정형 표현 7종 등) 은 [TONE_AND_MANNER.md](TONE_AND_MANNER.md) 에서 더 자세히 다룹니다. 본 절(고객 친화·구체성) 과 TONE_AND_MANNER 가 충돌하면 *사실 정합성(AUTHORING) > 고객 친화(8.4) > 세부 표현(TONE)* 순서로 우선합니다.
...
- 2026-05-29: 8.3 절 신설 — *문장·표현 출처는 base/ (stg 기준) 우선* 원칙. 새 매뉴얼 콘텐츠 작성 시 AI 가 처음부터 문장을 짜내지 말고, base/ 의 검증된 문장·서식·정형 패턴을 최대한 그대로 복사·재활용. xgen-main 처럼 base/ 를 직접 공유하는 customer 는 자동 충족. 사용자 피드백(2026-05-29) — claude 가 처음부터 작성한 한국어 문장이 매끄럽지 않다는 지적. 이전엔 휘발성 메모리에만 보관되던 규칙을 영속화.
- 2026-05-29: 8.4 절 신설 — *톤: 고객 친화·구체적 설명* 원칙. 모든 매뉴얼 본문은 고객(엔드 사용자) 시점에서 친절하고 이해하기 쉽게 구체적으로 설명. 고객 시점 동사 / 친절한 존댓말 / 구체적 라벨·진입 경로 / 추상 단어 풀어쓰기 / 숫자·시간·상태 명시 / 운영 권장사항·주의 케이스 / 시각 자료 보조 7개 규칙. 우선순위: 사실 정합성(AUTHORING) > 고객 친화(8.4) > 세부 표현(TONE). 사용자 피드백(2026-05-29) — 모든 매뉴얼 문체를 고객 대상으로 일관해야 한다는 요청.
