...
> **이 챕터는 관리 설정 > AI 거버넌스 > 통제 정책 관리 화면("AI 통제 정책")의 *개인정보보호(PII) 탭* 을 중심으로 설명합니다.** 같은 화면의 *금칙어* / *AI 위험도 등급* 탭은 절 끝에서 간단히 다룹니다. 외부 Guard 모델(예: Qwen3Guard-Gen vLLM) 호출 설정은 별도 화면이며 [가드레일 모델 설정](25b-guardrail-model.md) 챕터를 참고하세요. <!-- require_view: admin-system-guardrail -->
> **이 챕터는 관리 설정 > AI 거버넌스 > 통제 정책 관리 화면("AI 통제 정책")의 *개인정보보호(PII) 탭* 을 중심으로 설명합니다.** 같은 화면의 *금칙어* / *AI 위험도 등급* 탭은 절 끝에서 간단히 다룹니다. <!-- require_view: no-guardrail-mention -->
...
## 정책별 적용 대상 관리 { #policy-scope }

정책 목록의 **적용 대상** 열에서 **각 정책을 어떤 에이전트에 적용할지** 정책 단위로 관리합니다. 정책이 등록·활성화되어 있어도, 적용 대상으로 지정된 에이전트에서만 실제로 동작합니다.

- **적용 대상** 열 — 해당 정책이 현재 적용되는 범위를 표시합니다.
- **대상 지정** 버튼 — 이 정책을 적용할 대상 에이전트를 선택합니다.

**상태**(활성/비활성)가 *정책 자체의 사용 여부* 라면, **적용 대상**은 *그 정책이 어느 에이전트에 걸리는지 범위* 를 정하는 설정입니다. 예를 들어 주민등록번호 마스킹 정책을 특정 대고객 서비스 에이전트에만 적용하고 싶을 때 사용합니다.
{% endif %}
