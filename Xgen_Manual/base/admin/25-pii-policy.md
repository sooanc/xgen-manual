...
개인정보보호(PII) 정책은 AI가 입력과 출력 과정에서 개인식별정보를 자동으로 탐지하고 마스킹하여 개인정보 유출을 방지하는 기능입니다. 금융, 공공, 의료 등 개인정보 보호가 중요한 환경에서 AI 거버넌스의 핵심 정책으로 활용됩니다.

이 장에서는 관리 설정 > AI 거버넌스 > 통제 정책 관리 > **개인정보보호(PII) 탭**의 설정 방법과 운영 절차를 설명합니다. 금칙어 및 AI 위험도 등급 정책은 본 장 마지막에서 간략히 소개하며, Guard 모델 연동은 [가드레일 모델 설정](25b-guardrail-model.md)을 참고하시기 바랍니다. <!-- require_view: admin-system-guardrail -->
이 장에서는 관리 설정 > AI 거버넌스 > 통제 정책 관리 > **개인정보보호(PII) 탭**의 설정 방법과 운영 절차를 설명합니다. 금칙어 및 AI 위험도 등급 정책은 본 장 마지막에서 간략히 소개합니다. <!-- require_view: no-guardrail-mention -->
...
## 정책별 에이전트 적용 관리 { #policy-scope }

정책 목록의 **적용** 열에서 **각 정책을 어떤 에이전트에 적용할지** 정책 단위로 관리합니다. 정책이 등록·활성화되어 있어도, 적용 대상으로 지정된 에이전트에서만 실제로 동작합니다.

- **적용** 열 — 해당 정책이 현재 적용되는 범위를 표시합니다.
- **전체 적용** 버튼 — 클릭하면 **에이전트별 적용** 창이 열려, 이 정책을 적용할 에이전트를 지정합니다.

**전체 적용** 버튼을 누르면 **에이전트별 적용 : _정책명_** 창이 열립니다. 조직의 전체 에이전트가 목록으로 표시되고, 각 에이전트 우측의 **마스킹** 토글을 켜고 끄면 해당 정책을 그 에이전트에 적용할지 개별로 지정할 수 있습니다. 상단 **에이전트명 검색** 으로 특정 에이전트를 찾고, 목록이 길면 하단 페이지 이동으로 넘겨봅니다.

![에이전트별 적용 창 — 정책명별로 조직 전체 에이전트 목록과 에이전트별 마스킹 적용 토글, 상단 에이전트명 검색, 하단 페이지네이션](images/admin-gov-policy-agent-apply.png)

**상태**(활성/비활성)가 *정책 자체의 사용 여부* 라면, **적용**은 *그 정책이 어느 에이전트에 걸리는지 범위* 를 정하는 설정입니다. 예를 들어 주민등록번호 마스킹 정책을 특정 대고객 서비스 에이전트에만 적용하고 싶을 때 사용합니다.
{% endif %}
...
정책 저장 후 [AI 거버넌스 - 위험도 평가](29-governance-dashboard.md#risk-review) 에서 신규 에이전트가 자동 평가되며, 결과 등급에 따라 승인 대기 큐에 적재됩니다.
...
- [가드레일 모델 설정](25b-guardrail-model.md) — 외부 Guard 모델(Qwen3Guard 등) 호출 설정 (별도 화면) <!-- require_view: admin-system-guardrail -->
