...
!!! info "현재 stg 빌드의 모델 파라미터 노출"
    매뉴얼 이전 버전이 안내한 *Temperature / Max Tokens / Top P 슬라이더 화면* 은 현재 stg 라이브 빌드의 LLM 설정(`admin?view=admin-setting-llm`) 프로바이더 상세 페이지에 노출되지 않습니다. 위 표의 권장값은 에이전트플로우 LLM 노드의 *고급 옵션* 또는 호출 시 파라미터로 적용하세요. UI 노출이 추가되면 슬라이더 캡처와 함께 본 절을 보강합니다.
...
LLM 설정 관련 문의는 **XGen 관리자**({{vars.support_email}}) 로 연락해 주세요.
