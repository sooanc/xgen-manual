...
!!! note "일반 시스템 감사 로그와의 차이"
    ...
    어디로 가야 할지 헷갈리면: **"누가 / 언제 무엇을 했는가"** 가 사용자·시스템 단위면 시스템 감사 로그, **"이 에이전트는 어떻게 변해 왔는가"** 가 에이전트 단위면 본 화면을 사용하세요.

### Agent 상세 이력 — 6개 탭 { #audit-tracking-detail }

목록에서 **Agent 명** 을 클릭하면 해당 에이전트의 상세 이력 화면이 열리며, 상단에 첨부 파일 영역과 함께 다음 **6개 탭** 이 노출됩니다. 각 탭은 상단 stat 카드와 필터·기간 검색·CSV 다운로드 컨트롤을 공통으로 제공합니다.
...
!!! info "탭별 권장 활용"
    - **배포·거버넌스 승인 이력** — 이중 승인 워크플로의 단계별 처리 기록 조회. 회계·내부감사 응대 시 1차 자료.
    - **정책 변경 이력 / Agent 변경 이력** — 위험도 평가·통제 정책 변경의 책임 추적. 변경자·승인여부가 함께 기록되어 사고 발생 시 원인 시점을 좁힐 수 있습니다.
    - **데이터 접근정보** — RBAC 유형별 데이터 접근 빈도를 보고 권한 과부여를 식별.
