...
각 탭의 상단 헤더에는 *AI 서비스 변경 이력 목록* 으로 돌아가는 백 링크와 대상 Agent 의 식별 카드(위험등급·승인상태·점검상태·버전 배지) 가 고정 노출됩니다.

**탭 1 — 실행 상세 이력**

![실행 상세 이력 — 총 실행/성공/실패 카드와 실행 ID 단위 로그 테이블](images/admin-gov-audit-tab1-execution.png)

총 실행 건수 / 성공 / 실패 stat 카드 아래에 실행 ID · 실행일시 · 버전 · 실행유형 · 실행자 · 소요시간 · 상태 · 입력/출력 데이터 컬럼이 표시됩니다. *실행유형 전체* / *상태 전체* / *기간 검색* 필터와 CSV 내보내기를 함께 제공합니다.
