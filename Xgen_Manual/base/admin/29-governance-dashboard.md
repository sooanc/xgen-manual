...
|---|---|---|---|---|
| 1 | **AI 위험도 평가** | AI 위험도 평가 | [위험도 평가](#risk-review) | 위험 카테고리 위젯 그리드 → 임계치 초과 항목에서 *에이전트 승인* 화면으로 내부 이동 |
| 2 | **점검 이력 관리** | 점검 이력 및 계획 | [점검 이력 및 계획](#inspection) | 4개 탭 — 점검 이력 / 점검 계획 / 초과 점검 / 점검 이력 등록 |
...
!!! info "사이드바 라벨이 페이지 헤더와 다를 수 있습니다"
    예를 들어 *점검 이력 관리* 사이드바를 클릭하면 화면 헤더에는 **점검 이력 및 계획** 으로 표시됩니다. 본 챕터는 *사이드바 라벨* 을 기준으로 위치를 찾고, *페이지 헤더* 는 화면 진입 후 확인 기준으로 사용합니다.
...
## 점검 이력 및 계획 { #inspection }

거버넌스 심사가 완료된 에이전트의 **정기 점검 계획과 이력** 을 하나의 목록 화면에서 통합 관리합니다.

![점검 이력 및 계획 — 상단 5종 stat 카드, 검색·필터, 점검 대상 테이블](images/admin-gov-inspection-history.png)
...
| 기한초과여부 | **기한 초과 / 점검 예정 / 정상** 3가지 상태 배지 (기한 초과 시 초과일수, 점검 예정 시 `D-N` 표시) |
| 점검 등록 | 결과 등록 진입 액션 |
| 점검 이력 | 과거 점검 이력 모달 진입 |

!!! note "점검 예정일·점검 상태 판별 기준"
    목록의 **다음 점검일** 과 **기한초과여부** 배지는 다음 기준으로 결정됩니다.

    **다음 점검일** 은 *마지막 점검일* 에 등록된 *점검 주기* 를 더해 산출됩니다. 점검을 등록할 때 다음 점검일을 직접 지정하지 않으면 주기에 따라 자동으로 계산됩니다.
    ...
    **기한초과여부** 배지는 현재일(`{{지금}}`) 과 다음 점검일을 비교해 세 가지 상태로 표시됩니다.

    | 상태 | 표시 | 판별 기준 |
    |---|---|---|
    | 정상 | *정상* | 다음 점검일까지 5일을 초과해 남아 있는 경우 |
    | 점검 예정 | `D-5 점검 예정` | 다음 점검일이 5일 이내로 임박한 경우 — 다음 점검일 5일 전부터 표시 |
    | 기한 초과 | `+N일 미점검` | 다음 점검일이 현재일을 지난 경우. 초과 일수는 *현재일 − 다음 점검일* 로 계산 |

    즉 **점검 예정** 으로 잡히는 기준은 *다음 점검일까지 남은 기간이 5일 이내(D-5)* 인 시점입니다.
...
상단에는 총 실행 건수, 성공, 실패 현황을 요약한 대시보드 카드가 표시됩니다.
...
상단에는 총 접근 건수, 에이전트 개발자, 모델 개발자 기준의 접근 현황을 요약한 대시보드 카드가 표시됩니다.
...
상단에는 총 변경 건수, 승인완료, 승인대기, 승인반려 현황을 요약한 대시보드 카드가 표시됩니다.
...
상단에는 총 변경 건수, 자동 승인 완료 건수, 변경자 현황을 요약한 대시보드 카드가 표시됩니다.

하단 정책 변경 이력 테이블에서는 다음 정보를 확인할 수 있습니다.
...
또한 정책 변경 이력 분석을 위한 다중 필터 기능을 제공합니다.
...
해당 화면은 통제 정책 변경 이력과 변경 책임 추적 용도로 활용할 수 있습니다.

**탭 5 — 배포 승인 이력**

![배포 승인 이력 — 이중 승인 1단계 처리 기록](images/admin-gov-audit-tab5-deploy-approval.png)

이 화면은 이중 승인 절차 중 1차 단계인 시스템 관리자 배포 승인 이력을 조회하는 영역입니다.

상단에는 승인완료, 승인대기, 승인반려 상태별 대시보드 카드가 표시되며, 현재 승인 진행 현황을 빠르게 확인할 수 있습니다.

하단 이력 테이블에서는 다음 정보를 확인할 수 있습니다.
...
상단에는 승인완료, 승인보류, 승인반려 상태별 대시보드 카드가 표시되며, 거버넌스 검토 진행 현황을 확인할 수 있습니다.
