...
!!! info "현재 stg 빌드에 *+ MCP 서버 추가* 버튼은 노출되지 않음"
    매뉴얼 이전 버전이 안내한 *"+ MCP 서버 추가"* 버튼은 현재 stg 라이브 빌드의 MCP 라이브러리 화면(`admin?view=admin-mcp-market`) 우상단에 노출되지 않습니다. 화면은 *카탈로그 조회 + 카드 클릭 상세 진입* 흐름으로만 동작하며, 신규 서버 등록은 운영팀 측 별도 절차(설정 파일 / 배포 시점 인프라 등록)로 이뤄집니다. 카드 클릭 후 상세 페이지의 *설치* 흐름 캡처는 다음 회차에 보강 예정입니다.
...
## MCP 운영/모니터링 (Station) { #mcp-station }

**MCP 관리** 그룹의 두 번째 메뉴인 **MCP 운영/모니터링** (view ID `admin-mcp-station`) 은 등록된 MCP 서버들의 *런타임 상태* 와 *세션 라이프사이클* 을 한 화면에서 관리하는 곳입니다. *MCP 라이브러리* 가 *서버 카탈로그/등록* 이라면 본 화면은 *운영* 단계 입구입니다.

| 영역 | 표시 정보 |
|---|---|
| 서버별 상태 | 연결됨 / 끊김 / 오류 + 마지막 호출 시각 |
| 활성 세션 | 사용자별로 열려 있는 세션 목록 (계정, 시작 시각, 마지막 활동) |
| 호출 통계 | 시간대별 호출 횟수, 오류율, 평균 응답 시간 |
| 컨트롤 | 세션 개별 **종료** / **모두 종료** / 서버 **재시작** |

화면 진입: 관리 설정 → MCP 관리 → **MCP 운영/모니터링**.
...
MCP 라이브러리 관련 문의는 **XGen 관리자**({{vars.support_email}}) 로 연락해 주세요.
