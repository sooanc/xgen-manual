...
![관리 설정 진입 — Admin Control Center 화면. STEP 1 관리 영역 이해(거버넌스 담당자 / 시스템 관리자 책임 분담), STEP 2 주요 메뉴 선택(사용자/접근제어·Agent 운영·AI 거버넌스 등 9개 카드), 좌측에는 9개 관리 섹션 사이드바](images/admin-entry.png) <!-- require_view: admin-step2-9-areas -->
...
!!! note "SuperUser 권한 필요"
    관리 설정은 사용자 권한, Agent 운영, 시스템 환경, 데이터·지식 자산을 통합 관리하는 **SuperUser 전용 영역**입니다.
<!-- require_view_end -->
...
| 슈퍼유저 | SuperUser | 사용자/역할 관리, 시스템 설정, AI 거버넌스, 운영 모니터링, 권한 위임, 시스템 초기화 등 모든 관리 기능 | <!-- require_view: gov-monitoring -->
| 슈퍼유저 | SuperUser | 사용자/역할 관리, 시스템 설정, 운영 모니터링, 권한 위임, 시스템 초기화 등 모든 관리 기능 | <!-- require_view: no-governance -->

- 일반 사용자(Standard User) 계정이라도 운영 정책에 따라 SuperUser 가 특정 관리 메뉴 권한을 추가 부여할 수 있습니다.
...
| Agent 운영 | 노드 관리 | [에이전트 노드목록](../user/12a-node-list.md) |
...
| MCP 관리 | MCP 라이브러리 | [MCP 라이브러리 · MCP 라이브러리](28-mcp-market.md) | <!-- require_view: admin-mcp-market -->
...
환경 설정 그룹의 **전체 설정 / 인프라 / 오디오 / 사이드바** 4개 메뉴는 본 매뉴얼에 별도 챕터가 작성되어 있지 않습니다. 각 화면은 *시스템 설치/도메인별 1회 설정* 성격이라 솔루션 화면에서 직접 조작하는 것이 가장 빠릅니다.
<!-- require_view_end -->
<!-- require_view_start: env-overview-gs-2-menus -->
환경 설정 그룹의 **전체 설정 / 사이드바** 2개 메뉴는 본 매뉴얼에 별도 챕터가 작성되어 있지 않습니다. 각 화면은 *시스템 설치/도메인별 1회 설정* 성격이라 솔루션 화면에서 직접 조작하는 것이 가장 빠릅니다.
<!-- require_view_end -->

| 메뉴 | 화면 진입 | 화면 성격 |
|---|---|---|
| 전체 설정 | 관리 설정 → 환경 설정 → 전체 설정 (`admin?view=admin-system-config`) | 모든 시스템 구성 값을 *한 화면에서 통합 조회·편집* 하는 어드밴스드 뷰. 일반적인 운영은 *LLM / 검색·임베딩 / 가드레일* 등 **개별 메뉴** 를 사용하고, 본 화면은 *키-값 단위의 정밀 조회/편집* 용으로 사용합니다. (자세한 화면 구조는 아래 *전체 설정 화면 자세히* 참조) | <!-- require_view: admin-system-guardrail -->
| 전체 설정 | 관리 설정 → 환경 설정 → 전체 설정 (`admin?view=admin-system-config`) | 모든 시스템 구성 값을 *한 화면에서 통합 조회·편집* 하는 어드밴스드 뷰. 일반적인 운영은 *LLM / 검색·임베딩* 등 **개별 메뉴** 를 사용하고, 본 화면은 *키-값 단위의 정밀 조회/편집* 용으로 사용합니다. (자세한 화면 구조는 아래 *전체 설정 화면 자세히* 참조) | <!-- require_view: no-guardrail-mention -->
...
별도 챕터 신설은 후속 작업으로 예정되어 있으며, 그 전까지는 위 표의 진입 경로를 따라 솔루션 화면에서 직접 확인·설정하세요.
...
!!! warning "변경 전 영향 범위 확인 필수"
    *전체 설정* 은 LLM / 임베딩 / TTS / 노드 / 워크플로우 등 **솔루션 동작에 직접 영향을 주는 키** 들을 함께 노출합니다. 같은 키를 *LLM / 검색·임베딩 / 가드레일* 등 개별 메뉴에서도 편집할 수 있으므로, 어떤 화면에서 변경했는지 일관되게 기록하세요. 잘못된 값은 진행 중인 채팅·실행에 즉시 영향을 줍니다 — 변경 전 운영팀과 함께 검토를 권장합니다.
<!-- require_view_end -->
<!-- require_view_start: no-guardrail-mention -->
!!! warning "변경 전 영향 범위 확인 필수"
    *전체 설정* 은 LLM / 임베딩 / 노드 / 워크플로우 등 **솔루션 동작에 직접 영향을 주는 키** 들을 함께 노출합니다. 같은 키를 *LLM / 검색·임베딩* 등 개별 메뉴에서도 편집할 수 있으므로, 어떤 화면에서 변경했는지 일관되게 기록하세요. 잘못된 값은 진행 중인 채팅·실행에 즉시 영향을 줍니다 — 변경 전 운영팀과 함께 검토를 권장합니다.
<!-- require_view_end -->
...
관리자 기능 관련 문의는 Xgen 솔루션 관리자에게 문의해 주세요.
