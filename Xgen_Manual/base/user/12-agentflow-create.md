...
> 사용 가능한 전체 노드 카탈로그(카테고리, 태그, 상세 스펙 등) 와 각 노드의 입출력·파라미터 상세 명세는 바로 다음 [에이전트 노드목록](12a-node-list.md) 챕터에서 확인할 수 있습니다.
...
처음 사용하거나 새로운 패턴을 빠르게 익히고 싶을 때, 캔버스 좌상단 **새 에이전트플로우 ▾** 드롭다운을 열고 **튜토리얼 가이드로 시작** 을 선택하면 *튜토리얼 선택* 패널이 열립니다. 패널은 두 개의 탭으로 구성됩니다.
...
![튜토리얼 선택 — 캔버스 좌상단 *새 에이전트플로우 ▾ → 튜토리얼 가이드로 시작* 으로 열리는 *튜토리얼 선택* 패널의 기본 튜토리얼 탭](images/tutorial-panel.gif)
...
**Agent 목록** 의 **템플릿** 에서 *공유튜토리얼 등록* 으로 추가한 항목만 노출됩니다. 자주 쓰는 사내 템플릿을 공유튜토리얼로 묶어두면, 다음에 같은 패턴을 만들 때 단계별로 재생할 수 있습니다.

![공유 튜토리얼 — 튜토리얼 선택 패널에서 *공유 튜토리얼* 탭으로 전환했을 때 보이는 화면](images/tutorial-template-tab.gif)

등록 방법:

1. 좌측 사이드바에서 **Agent 작업실 → Agent 목록** 으로 이동한 뒤 상단 **템플릿** 탭을 선택합니다.
2. 공유튜토리얼로 사용할 템플릿 카드에서 **공유튜토리얼 등록** 을 클릭합니다.
3. 캔버스 좌상단 **새 에이전트플로우 ▾ → 튜토리얼 가이드로 시작** 으로 패널을 열고 **공유 튜토리얼** 탭에서 확인합니다.

해제는 같은 템플릿 카드의 **공유튜토리얼 해제** 버튼으로 가능합니다.

!!! note "시연할 단계가 없는 템플릿은 등록 불가"
    노드가 없어 시연할 단계가 없는 템플릿은 공유튜토리얼로 등록되지 않습니다.
