...
![캔버스 편집기 — 빈 캔버스의 "에이전트 시작" 버튼을 클릭하면 XGEN Agent 노드가 자동 배치되는 흐름](images/canvas-editor.gif)
...
- Basic Chatbot — 가장 단순한 챗봇 구성
- Translation Bot — 다국어 번역
- Email Writer — 이메일 초안 생성
- Prompt Template — 프롬프트 템플릿 활용
- Internal RAG QA — 사내 문서 기반 질의응답
- Multilingual Support — 다국어 응답
- Daily News Briefing Mailer — 뉴스 요약 메일링
- News Analysis with Self-Eval — 자기 평가가 포함된 뉴스 분석
...
본인이 **에이전트플로우 라이브러리(스토어)** 에서 *가상튜토리얼 등록* 으로 명시적으로 추가한 항목만 노출됩니다. 자주 쓰는 사내 템플릿을 가상튜토리얼로 묶어두면, 다음에 같은 패턴을 만들 때 단계별로 재생할 수 있습니다.
...
에이전트 만들기 관련 문의는 **XGen 관리자**({{vars.support_email}}) 로 연락해 주세요.
