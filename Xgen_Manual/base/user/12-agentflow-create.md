...
에이전트플로우는 여러 노드(Node)를 시각적으로 연결하여 정의한 AI 작업 흐름입니다. 시작 노드 → 중간 처리(LLM 호출, 도구 실행, 분기 등) → 응답까지의 단계가 한 다이어그램으로 표현됩니다.
...
!!! note "Agent 제작 권한 필요"
    본 절차는 **Agent 개발자** 역할(또는 동등한 `main.agentflow:*` 권한)을 부여받은 계정에서 따라야 합니다. 일반 사용자(Standard User) 계정에는 사이드바에 **Agent 제작** 섹션이 노출되지 않으며, [대시보드 · Agent 작업실 단축 버튼](18-dashboard.md) 으로 `/main?view=canvas-intro` 에 진입해도 페이지 본문이 비어 보입니다.
...
!!! note "노드 사이드바 드래그 화면 캡처는 별도 갱신 예정"
    캔버스 좌측 노드 팔레트에서 노드 카드를 드래그하는 순간의 캡처는 다음 회차에 보강 예정입니다. 위 캔버스 편집기 이미지에서 좌측 노드 패널 위치를 확인할 수 있습니다.
...
!!! note "노드 상세 패널 캡처는 별도 갱신 예정"
    개별 노드를 클릭했을 때 우측에 펼쳐지는 상세 설정 패널의 캡처는 다음 회차에 보강 예정입니다.
