...
본 챕터는 만든 에이전트플로우를 실행·배포·공유·버전 관리하는 절차를 다룹니다. <!-- require_view: agentflow-version-control -->
본 챕터는 만든 에이전트플로우를 실행·배포·공유하는 절차를 다룹니다. <!-- require_view: no-agentflow-version-control -->

## 실행과 디버깅 { #testing }

### 캔버스에서 즉시 실행

1. 캔버스 좌측 하단 **실행** 버튼 클릭
2. 입력값 모달에서 테스트 입력 제공
3. 캔버스 하단 패널에 실행 결과·로그 표시

![에이전트 실행 — 캔버스 좌측 하단 실행 버튼 클릭 후 하단 패널에서 실행 결과·로그 확인](gif/20 canvas agent running.gif)
...
## 배포(*UI/UX 작업이후 업데이트 예정) { #deployment }

본 챕터에서는 에이전트가 실제 사용자에게 서비스되기까지의 **전체 배포 흐름** 을 설명합니다. Agent 개발자가 직접 수행하는 단계는 **배포 요청** 까지이며, 이후 운영 정책에 따라 관리자 승인 절차를 거친 후 운영 환경에 배포됩니다.
...
4. 모달 상단의 **배포 토글**(*비공개 ↔ 배포 중*)을 ON 으로 전환합니다. 이 토글이 **배포 요청을 보내는 단일 트리거** 입니다.
   공유 배포 시에는 토글이 켜지기 전에 **Agent 개발 기획서** 를 먼저 선택해야 하며, 미선택 상태로 토글을 켜면 *"공유 배포에는 Agent 개발 기획서 선택이 필요합니다."* 오류가 노출됩니다. <!-- require_view: admin-agent-dev-plan -->
...
!!! info "모달 캡처는 다음 회차"
    배포 토글, 모드 탭(웹페이지/API/cURL/임베드), Agent 개발 기획서 선택기를 보여주는 모달 캡처는 본인 소유 에이전트가 있는 계정으로 다음 회차 캡처 예정입니다. 본 절은 동작 설명을 솔루션 화면 흐름에 맞춰 정확히 기술하므로 캡처 부재가 절차 이해에 영향을 주지 않습니다. <!-- require_view: admin-agent-dev-plan -->
    배포 토글, 모드 탭(웹페이지/API/cURL/임베드)을 보여주는 모달 캡처는 본인 소유 에이전트가 있는 계정으로 다음 회차 캡처 예정입니다. 본 절은 동작 설명을 솔루션 화면 흐름에 맞춰 정확히 기술하므로 캡처 부재가 절차 이해에 영향을 주지 않습니다. <!-- require_view: no-agent-dev-plan -->
...
### 이후 흐름 — 시스템 관리자 배포 승인 { #dual-approval-flow }

배포 토글을 켠 *순간* 부터 사용자 검색·실행에 노출되는 것이 아니라, **시스템 관리자의 배포 승인** 을 통과해야 서비스가 활성화됩니다.

| 단계 | 담당 | 위치 | 결과 |
|---|---|---|---|
| 0. 배포 요청 | **본인 (Agent 개발자)** | Agent 목록 → 카드 dropdown → 배포 정보 → 배포 토글 ON | 카드 배지 *"배포 대기"* (`inquire_deploy: true`) |
| 1. 배포 승인 | **시스템 관리자** | 관리 설정 → Agent 운영 → Agent 관리 | 카드 배지 *"배포됨"* (`is_accepted: true`, `is_deployed: true`) — 자세한 절차는 [관리자 매뉴얼 · Agent 관리 — 배포 승인](../admin/32-agent-operations.md#agent-mgmt-deploy-approval) |
| ✅ 서비스 가능 | — | 배포 승인이 완료된 시점부터 사용자 검색·실행에 노출 | — |

진행 상황은 [대시보드 · Agent 배포/승인 상태](18-dashboard.md) 위젯에서 *배포 승인 대기 → 승인완료* 흐름으로 직접 추적할 수 있습니다. 시스템 관리자가 거부하면 카드 배지가 *"미배포"* 로 복귀하므로, 사유를 확인한 뒤 보완해 다시 요청합니다.

!!! warning "거부 사유 확인 경로"
    **시스템 관리자 거부** 는 작성자에게 별도 채널(메신저·메일)로 통보되는 게 일반적입니다. 사내 운영 정책에 따라 달라지므로 사전에 확인해 두세요.
<!-- require_view_end -->
...
- **버전은 의미 있는 시점에 명시 저장** — 큰 변경 전후로 명시적으로 저장하면 되돌리기 쉬움 <!-- require_view: agentflow-version-control -->

## 문의

에이전트 운영 관련 문의는 Xgen 솔루션 관리자에게 문의해 주세요.
