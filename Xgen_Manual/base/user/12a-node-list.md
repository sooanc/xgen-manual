...
| 파라미터 | Prompt Source | STR | — | 시스템 프롬프트 제공 방식 선택. `direct` → 아래 System Prompt 에 직접 작성, `template` → 저장된 프롬프트 템플릿에서 선택. |
| 파라미터 | Template | STR | — | 저장된 프롬프트 템플릿 선택. `System Prompt` 타입만 표시. `prompt_source` 가 `template` 일 때 활성화되며 선택 후 내용 수정 가능. |
| 파라미터 | System Prompt | STR | — | AI 의 역할과 지시사항을 작성합니다. 예) `You are a friendly financial advisor. Answer loan and deposit questions accurately.` |
