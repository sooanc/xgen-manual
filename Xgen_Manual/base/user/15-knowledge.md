...
!!! note "컬렉션 생성 모달 캡처는 별도 갱신 예정"
    "+ 새 컬렉션 생성" 클릭 시 노출되는 모달(이름·설명·암호화·만료일자 입력)의 캡처는 다음 회차에 보강 예정입니다.
...
DB 연동 시 각 테이블·컬럼에 대한 문서화(설명·예시·정책)는 별도 화면에서 관리합니다.

![DB 문서화 — 테이블·컬럼별 설명과 예시값 관리](images/database-documentation.png)
