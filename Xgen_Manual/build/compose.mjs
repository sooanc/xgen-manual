...
export async function compose(customerId) {
  ...
  //     .en.md 는 i18n suffix 변환 — base 챕터(.md)와 같은 nav 엔트리이므로 별도 처리 불필요.
  const screenTruth = await loadScreenTruth();
  const excludedByDir = new Map(); // absDir → Set<basename>
  for (const f of mdFiles) {
    ...
    if (!excludedByDir.has(dir)) excludedByDir.set(dir, new Set());
    excludedByDir.get(dir).add(name);
    console.log(
      `[compose:${customerId}] hide from nav: ${relative(docsDir, f)} (require_view: ${wanted.join(', ')}) — stg 에서 미확인`
    );
  }

  // 6b. 제외 대상이 있는 디렉토리마다 .pages 파일 생성 (awesome-pages nav: 명시)
  //     이미 .pages 가 존재하는 디렉토리는 건드리지 않음 — 수작업 nav 우선.
  //     .pages 가 없는 디렉토리만 자동 생성 (제외 항목을 뺀 알파벳 순 리스트).
  for (const [dir, excludedNames] of excludedByDir) {
    const pagesPath = join(dir, '.pages');
    if (existsSync(pagesPath)) {
      console.log(
        `[compose:${customerId}] skip auto .pages (이미 존재): ${relative(docsDir, pagesPath)} — 수동 nav 가 우선합니다. 제외 항목: ${[...excludedNames].join(', ')}`
      );
      continue;
    }
    const entries = await readdir(dir, { withFileTypes: true });
    ...
    for (const e of entries.sort((a, b) => a.name.localeCompare(b.name))) {
      if (e.isDirectory()) {
        ...
        navEntries.push(e.name);
        continue;
      }
      if (!e.name.endsWith('.md')) continue;
      // i18n suffix(.en.md) 는 base 챕터(.md)의 언어 변형 — nav 엔트리는 base 만 명시.
      if (/\.en\.md$/.test(e.name)) continue;
      if (excludedNames.has(e.name)) continue;
      navEntries.push(e.name);
    }
    const yaml = `# Auto-generated by compose.mjs — screen-truth 기반 nav 제외\n# 제외 챕터(.md): ${[...excludedNames].join(', ')}\nnav:\n${navEntries.map((n) => `  - ${n}`).join('\n')}\n`;
    await writeFile(pagesPath, yaml, 'utf8');
  }
